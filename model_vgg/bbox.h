...
struct AssignResult
{
    ...
    torch::Tensor assigned_inds; // -2: ignore, -1: negative, >=0: indices(not gt labels) of gt_bboxes in current image
    torch::Tensor gt_bboxes;     // [[1, 1, 10, 10], ..., [2, 2, 20, 20], [3, 3, 30, 30]]
    torch::Tensor gt_labels;     // [1, ..., 2, 3]
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> multiclass_nms(torch::Tensor bboxes, // [n, #class*4] or [n, 4]
                                                                       ...
                                                                       float iou_thr, float score_thr = -1,
                                                                       int max_num = -1);
