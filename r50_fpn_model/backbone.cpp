...
torch::nn::Conv2d conv3x3(int64_t in, int64_t out, int64_t stride, int64_t groups)
{
...
BasicBlock::BasicBlock(int64_t inplanes, int64_t planes, int64_t stride, torch::nn::Sequential downsample,
                       int64_t groups, int64_t base_width)
    : stride(stride), downsample(downsample)
{
    assert(groups == 1 && base_width == 64 && "BasicBlock only supports groups=1 and base_width=64");
...
Bottleneck::Bottleneck(int64_t inplanes, int64_t planes, int64_t stride, torch::nn::Sequential downsample,
                       int64_t groups, int64_t base_width)
    : stride(stride), downsample(downsample)
{
...
ResNet18Impl::ResNet18Impl(int frozen_stages) : ResNetImpl({2, 2, 2, 2}, false, 1, 64, frozen_stages)
{
}

ResNet34Impl::ResNet34Impl(int frozen_stages) : ResNetImpl({3, 4, 6, 3}, false, 1, 64, frozen_stages)
{
}

ResNet50Impl::ResNet50Impl(int frozen_stages) : ResNetImpl({3, 4, 6, 3}, false, 1, 64, frozen_stages)
{
}

ResNet101Impl::ResNet101Impl(int frozen_stages) : ResNetImpl({3, 4, 23, 3}, false, 1, 64, frozen_stages)
{
}

ResNet152Impl::ResNet152Impl(int frozen_stages) : ResNetImpl({3, 8, 36, 3}, false, 1, 64, frozen_stages)
{
}

ResNext50_32x4dImpl::ResNext50_32x4dImpl(int frozen_stages) : ResNetImpl({3, 4, 6, 3}, false, 1, 64, frozen_stages)
{
}

ResNext101_32x8dImpl::ResNext101_32x8dImpl(int frozen_stages) : ResNetImpl({3, 4, 23, 3}, false, 1, 64, frozen_stages)
{
}
...
std::shared_ptr<Backbone> build_backbone(const boost::property_tree::ptree &backbone_opts)
{
    ...
    {
        std::cout << "[build_backbone] building resnet50, frozen_stages: " << backbone_opts.get<int>("frozen_stages")
                  << std::endl;
        return std::make_shared<ResNet50Impl>(backbone_opts.get<int>("frozen_stages"));
    }
    else
    {
        throw std::runtime_error("unsupported backbone type: " + backbone_type);
    }
}
} // namespace backbone
