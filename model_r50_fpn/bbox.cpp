...
torch::Tensor BBoxRegressCoder::encode(torch::Tensor base, torch::Tensor bboxes)
{
    ...
    auto wh_delta = torch::log(bboxes_xywh.index({Slice(), Slice(2, None)}) / base_wh);
...
torch::Tensor BBoxRegressCoder::decode(torch::Tensor base, torch::Tensor delta, const std::vector<int64_t> &max_shape)
{
    base = base.to(torch::kFloat32);
    ...
    auto base_xywh = xyxy2xywh(base);
    auto base_wh = base_xywh.index({Slice(), Slice(2, None)});
    auto bboxes_xy = delta.index({Slice(), Slice(None, 2)}) * base_wh + base_xywh.index({Slice(), Slice(None, 2)});
...
BBoxAssigner::BBoxAssigner(float pos_iou_thr, float neg_iou_thr, float min_pos_iou, int samp_num, float pos_frac,
                           bool add_gt)
    : _pos_iou_thr(pos_iou_thr), _neg_iou_thr(neg_iou_thr), _min_pos_iou(min_pos_iou), _samp_num(samp_num),
      _pos_frac(pos_frac), _add_gt(add_gt)
{
    ...
    std::cout << "[BBoxAssigner] pos_iou_thr: " << _pos_iou_thr << ", neg_iou_thr: " << neg_iou_thr
              << ", min_pos_iou: " << _min_pos_iou << ", samp_num: " << _samp_num << ", pos_frac" << _pos_frac
...
AssignResult BBoxAssigner::assign(torch::Tensor bboxes, torch::Tensor gt_bboxes, torch::Tensor gt_labels)
{
    ...
    if (_add_gt)
    {
        bboxes = torch::cat({gt_bboxes, bboxes}, 0);
    }
    ...
    assigned_inds.index_put_({max_gt_iou < _neg_iou_thr},
                             -1); // find negative inds
    assigned_inds.index_put_({max_gt_iou >= _pos_iou_thr},
                             1); // find positive inds
    ...
    max_gt_idx.index_put_({equal_max_places}, equal_max_idx.index({equal_max_places})); // modified equal_max_places to
                                                                                        // {equal_max_places}
    //
...
torch::Tensor calc_iou(torch::Tensor a, torch::Tensor b)
{
    ...
    a = a.view({-1, 1, 4});
    // top-left
    auto tl = torch::max(a.index({Slice(), Slice(), Slice(None, 2)}), b.index({Slice(), Slice(None, 2)}));
    // bottom-right
    auto br = torch::min(a.index({Slice(), Slice(), Slice(2, None)}), b.index({Slice(), Slice(2, None)}));
    auto inter_wh = br - tl;
    auto pos_mask =
        torch::logical_and((inter_wh.index({Slice(), Slice(), 0}) > 0), (inter_wh.index({Slice(), Slice(), 1}) > 0));
    auto inter_area = inter_wh.index({Slice(), Slice(), 0}) * inter_wh.index({Slice(), Slice(), 1});
...
torch::Tensor elem_iou(torch::Tensor a, torch::Tensor b)
{
    assert(a.sizes() == b.sizes() && a.dim() == 2 && b.dim() == 2 && a.size(1) == 4 && b.size(1) == 4 &&
           "in order to calculate element-wise iou, bboxes must have same and "
           "valid dimensions");
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> multiclass_nms(torch::Tensor bboxes, // [n, #class*4] or [n, 4]
                                                                       ...
                                                                       float iou_thr, float score_thr, int max_num)
{
