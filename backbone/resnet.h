...
struct BasicBlock : torch::nn::Module
{
    ...
    BasicBlock(int64_t inplaces, int64_t places, int64_t stride = 1, torch::nn::Sequential downsample = nullptr,
...
struct ResNetImpl : Resnet
{
    ...
    ResNetImpl(const std::vector<int> &layers, bool zero_init_residual = false, int64_t groups = 1,
               int64_t width_per_groups = 64, int64_t frozen_stages = 1);
...
template <typename Block>
torch::nn::Sequential ResNetImpl<Block>::_make_layer(int64_t planes, int64_t blocks, int64_t stride)
{
...
template <typename Block>
ResNetImpl<Block>::ResNetImpl(const std::vector<int> &layers, bool zero_init_residual, int64_t groups,
                              int64_t width_per_group, int64_t frozen_stages)
    : groups(groups), base_width(width_per_group), inplanes(64), frozen_stages(frozen_stages),
      conv1(torch::nn::Conv2dOptions(3, 64, 7).stride(2).padding(3).bias(false)), bn1(64),
      layer1(_make_layer(64, layers[0])), layer2(_make_layer(128, layers[1], 2)),
      layer3(_make_layer(256, layers[2], 2)), layer4(_make_layer(512, layers[3], 2))
{
