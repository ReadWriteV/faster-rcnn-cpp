...
RPNHeadImpl::RPNHeadImpl(int in_channels, int feat_channels, const boost::property_tree::ptree &anchor_opts,
    ...
    : _in_channels(in_channels), _feat_channels(feat_channels), _anchor_opts(anchor_opts),
      _bbox_coder_opts(bbox_coder_opts), _loss_cls_opts(loss_cls_opts), _loss_bbox_opts(loss_bbox_opts),
      ...
      _bbox_assigner(train_opts.get_child("bbox_assigner_opts"))
{
    ...
    _conv = torch::nn::Conv2d(torch::nn::Conv2dOptions(in_channels, feat_channels, 3).stride(1).padding(1));
    _classifier = torch::nn::Conv2d(
        torch::nn::Conv2dOptions(feat_channels, _anchor_generator->num_anchors() * _class_channels, 1));
    _regressor = torch::nn::Conv2d(torch::nn::Conv2dOptions(feat_channels, _anchor_generator->num_anchors() * 4, 1));
...
RPNHeadImpl::RPNHeadImpl(const boost::property_tree::ptree &opts)
    : RPNHeadImpl(opts.get<int>("in_channels"), opts.get<int>("feat_channels"), opts.get_child("anchor_opts"),
                  opts.get_child("bbox_coder_opts"), opts.get_child("loss_cls_opts"), opts.get_child("loss_bbox_opts"),
                  opts.get_child("train_opts"), opts.get_child("test_opts"))
{
}

std::tuple<std::vector<torch::Tensor>, std::vector<torch::Tensor>> RPNHeadImpl::forward(
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> RPNHeadImpl::forward_train(
    std::vector<torch::Tensor> feats, const dataset::DetectionExample &example)
{
    ...
    auto grid_sizes = utils::get_grid_size(feats);
    ...
    cls_out_list = utils::batch_permute(cls_out_list, {0, 2, 3, 1});
    bbox_out_list = utils::batch_permute(bbox_out_list, {0, 2, 3, 1});
    cls_out_list = utils::batch_reshape(cls_out_list, {-1, _class_channels});
    bbox_out_list = utils::batch_reshape(bbox_out_list, {-1, 4});
    anchor_list = utils::batch_reshape(anchor_list, {-1, 4});

    /*************** get proposals *************/
    auto proposals = get_proposals(anchor_list, cls_out_list, bbox_out_list, example.img_shape, _train_opts);
    /*************** get proposals *************/
...
torch::Tensor RPNHeadImpl::get_proposals(std::vector<torch::Tensor> anchor_list,   // [n, 4]
...
{
    assert(cls_out_list.size() == bbox_out_list.size() && "cls_outs and bbox_outs must have same number of levels");
    ...
    for (int i = 0; i < cls_out_list.size(); i++)
    {
        ...
        lvls_idx.push_back(torch::full({cls_score.size(0)}, i, torch::kInt64).to(cls_score.device()));
    }
    ...
    auto keep = bbox::batched_nms(all_bbox, all_score, all_idx, opts.get<float>("nms_thr"));
    // proposals are [n, 5] tensors where last index is score
    auto proposals = torch::cat({all_bbox.index({keep}), all_score.index({keep}).view({-1, 1})}, 1);
...
torch::Tensor RPNHeadImpl::forward_test(std::vector<torch::Tensor> feats, const dataset::DetectionExample &img_data)
{
    auto rpn_outs = forward(feats);
    auto cls_out_list = std::get<0>(rpn_outs), bbox_out_list = std::get<1>(rpn_outs);
    auto anchor_list = _anchor_generator->get_anchors(utils::get_grid_size(feats));
    cls_out_list = utils::batch_permute(cls_out_list, {0, 2, 3, 1});
    bbox_out_list = utils::batch_permute(bbox_out_list, {0, 2, 3, 1});
    cls_out_list = utils::batch_reshape(cls_out_list, {-1, _class_channels});
    bbox_out_list = utils::batch_reshape(bbox_out_list, {-1, 4});
    anchor_list = utils::batch_reshape(anchor_list, {-1, 4});
    /*************** get proposals *************/
    return get_proposals(anchor_list, cls_out_list, bbox_out_list, img_data.img_shape, _test_opts);
}
